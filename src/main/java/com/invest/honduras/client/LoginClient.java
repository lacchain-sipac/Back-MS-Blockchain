...
import com.invest.honduras.util.GlobalException;
...
public class LoginClient {
	...
	public Mono<LoginResponse> refreshToken(String token) {
		...
		clientResponse = this.webClient.post().uri(Constant.API_URL_REFRESH_TOKEN).accept(MediaType.APPLICATION_JSON)
				.header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE)
				// .body(Mono.just(token), String.class)
				.header(HttpHeaders.AUTHORIZATION, token).exchange().doOnError(e -> {
					...
					if (response.statusCode().equals(HttpStatus.UNAUTHORIZED))
						...
						throw new GlobalException(HttpStatus.INTERNAL_SERVER_ERROR,
								Constant.MESSAGE_REFRESH_TOKEN_ERROR);
